...
import { numbersService } from '../services/cms-crud.service'
...
export function NumbersPage() {
  ...
  const fetchData = async () => {
    setIsLoading(true)
    try {
      const res = await numbersService.findAll({ page, search: search || undefined })
      setData(res.data)
  ...
  const handleSave = async () => {
    const body = { number: formNumber, word: formWord, difficulty: formDifficulty, language: formLanguage, isActive: formIsActive }
    if (editingItem) {
      await numbersService.update(editingItem.id, body)
    } else {
      await numbersService.create(body)
    }
    setModalOpen(false)
    fetchData()
  }

  const handleToggle = async (item: NumberItem) => {
    await numbersService.toggleStatus(item.id, !item.isActive)
    fetchData()
  }

  const handleDelete = async (item: NumberItem) => {
    await numbersService.remove(item.id)
    fetchData()
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-800">Numbers</h1>
        {canEdit && (
      ...
      <CmsModal isOpen={modalOpen} onClose={() => setModalOpen(false)} title={editingItem ? 'Edit Number' : 'Create Number'}>
        <div className="space-y-3">
          <div>
            <label className="block text-sm font-medium text-gray-700">Number</label>
            <input value={formNumber} onChange={(e) => setFormNumber(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2 text-sm" />
          </div>
          <div>
